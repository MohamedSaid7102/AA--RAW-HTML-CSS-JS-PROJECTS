...
function showTime(){
  ...
  // show the time
  time.innerHTML = `${Hour}<span>:</span>${showZero(Min)}<span>:</span>${showZero(Sec)} <span id="amPm">${amPm}</span>`;
  // 12H format 
  Hour = Hour % 12 || 12;
...
// set The name
function setName(e){
  if(e.type === 'keypress'){ // it's a key
    if(e.which === 13 || e.keyCode === 13){ // if it's an Enter
...
// set the focus
function setFocus(e){
  if(e.type === 'keypress'){ //check if the event is keypress
    if(e.which === 13 || e.keyCode === 13){ // if the key is enter only
      localStorage.setItem('focus',e.target.innerText);
      focus.blur();
    }
  }else{ // the event is a blur 'click outside'
    localStorage.setItem('focus',e.target.innerText);
  }
}
